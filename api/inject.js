...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ★ ② 는 라벨만 바꾼다. 「No.1」 은 옆 두 칸(14+ · 28)과 같은 큰 숫자라 남겨야 3열 리듬이 산다.
    // ★ tracking 0.22em 은 대문자 영문용이라 한글에 쓰면 자간이 벌어져 깨진다 → 0.02em + break-keep.
    // ★ JS 오버레이가 아니라 여기서 치환하는 이유: 오버레이는 DOMContentLoaded 뒤에 돌아
    //   옛 문구가 한 번 번쩍인다. 항목4(FOUC) 로 이미 지적받은 그 증상이다.
    // 앵커 2개 모두 index_v6.html 안에서 유일함을 실측했다(grep -c → 각 1).
    const S01 = [
      ...
      ['<div class="text-[10px] font-bold tracking-[0.22em] text-white/40 mt-2">IN KOREA</div>',
       '<div class="text-[10px] font-bold tracking-[0.02em] text-white/40 mt-2 break-keep">최초의 혁신적 후드</div>']
    ];
