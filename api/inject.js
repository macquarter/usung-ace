...
export default async function handler(req, res) {
  ...
  try {
    ...
    //
    // ── r43 — 3번 칸을 다시 「숫자 + 라벨」 2단으로 되돌린다 ──
    // r32 는 큰 칸 하나가 문구 전체를 지게 만들었다. 그 결과 세 칸의 생김새가
    // 「48px 숫자 + 10px 라벨」 / 「48px 숫자 + 10px 라벨」 / 「16px 문구, 라벨 없음」 이 되어
    // 3열 리듬이 깨졌다(393px 실측: 1·2번 칸 자식 2개, 3번 칸 자식 1개).
    // 승연 요청 — 「최초의 혁신적 후드 옆에 내용처럼 숫자와 최초의 혁신적 후드가 들어갔으면」.
    //   → 숫자는 위, 문구는 **라벨 자리**로. 옆 두 칸과 클래스를 글자 하나까지 같게 준다.
    // ★ 숫자는 「1」 이다. 「최초」 를 숫자로 옮긴 것이지 순위 주장이 아니다.
    //   r3 덱이 지운 건 「No.1 / IN KOREA」 라는 **국내 1위 주장**이었고, 여기 라벨은
