/* usung-r23.js — 부품소개 페이지(51종) 배치·번호를 유성 「부품 크롭본」 폴더와 일치시킨다.
...
(function () {
  ...
  /* [크롭본 번호(표시 그대로), 리포 부품 id] — 51행. 순서가 곧 화면 배치 순서다.
     260816 추가 3행: 01-1(p52 반후지 150Ø) · 01-2(p53 태엽용 반후지) · C(p54 코브라 롱망).
     뱃지는 PN 표 키를 그대로 쓴다 — p54 는 크롭본 폴더에 번호가 없어 PN 도 'C' 다. */
  var ORDER = [
    ['01', 'p01'], ['01-1', 'p52'], ['01-2', 'p53'],
  ...
  /* renderPartsPage 를 감싼다. 원본을 먼저 돌려 머리말(PARTS & COMPONENTS · 51종 설명)을
     그대로 물려받고, 그리드만 우리 순서로 다시 그린다.
     ★ 머리말의 「51종」 숫자는 여기서 안 고친다. usung-r8-prod-a.js / usung-r8-view.js 의
       원문과 usung-r16-i18n-{a,c,d}.js 의 한국어 키를 **같이** 51 로 맞춰 뒀다.
