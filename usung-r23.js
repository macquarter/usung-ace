/* usung-r23.js — 부품소개 페이지(50종) 배치·번호를 유성 「부품 크롭본」 폴더와 일치시킨다.
 * 260816) 크롭본 48종 + 260809 추가 3종(p52·p53·p54) = 51종.
 * 260818 r45) 「01-1 반후지 150Ø」 삭제 → 50종. 「01 반후지 150·125Ø」가 이미 150Ø 를 덮는다
 *   (r3_라스트 덱 S12 「두개가 동일한 것이어서 01-1을 삭제해주세욤」).
 *   같은 덱 S11 로 뱃지 'C'(코브라 롱망) → '45' 로 바꿔 맨 뒤에 배치.
 *
 * 왜 필요한가
 ...
 *   ★★ r45 예외 — 'C' → '45' 는 ORDER 에서만 한다. PN 의 키는 **PPT 번호**이고
 *      (파일 머리말: 「PPT 번호 != 리포 파일 번호, 변환은 PN 표 한 곳에서만」)
 *      r19 원본 덱 S32 가 그 부품을 'C' 로 부른다. PN 키를 45 로 고치면 그 덱과 어긋나고,
 *      L 의 COB_R:'C' 까지 연쇄로 고쳐야 한다. PN 키는 화면에 절대 안 나온다 —
 *      뱃지 글자는 오직 이 ORDER 의 첫 칸이다. 그래서 표시 번호만 여기서 바꾼다.
 *      같은 이유로 '01-1' 삭제도 ORDER 한정이다. PN['1-1'] 과 모델별 조성 리스트
 *      (GA_S…GD_L 9개)는 그대로 둔다 — 그건 「모델에 150Ø 반후지가 들어간다」는
 *      규격 정보라 갤러리 중복과 다른 사실이다.
 *
 * 건드리지 않는 것
...
(function () {
  ...
  /* [화면 표시 번호, 리포 부품 id] — 50행. 순서가 곧 화면 배치 순서다.
     260816 추가분 중 남은 2행: 01-2(p53 태엽용 반후지) · 45(p54 코브라 롱망).
     ★ p52(반후지 150Ø)는 r45 에서 뺐다 — 01(반후지 150·125Ø)과 같은 부품이다.
     ★ p54 뱃지는 r45 부터 '45' — 크롭본 폴더에 번호가 없어 PN 이 'C' 로 부르던 것을
       「맨 마지막 넘버」로 바꿔 44 다음에 놓았다. PN 키('C')는 그대로다(위 머리말 참고). */
  var ORDER = [
    ['01', 'p01'], ['01-2', 'p53'],
    ['02', 'p02'], ['03', 'p03'], ['04', 'p04'],
  ...
  /* renderPartsPage 를 감싼다. 원본을 먼저 돌려 머리말(PARTS & COMPONENTS · 50종 설명)을
     그대로 물려받고, 그리드만 우리 순서로 다시 그린다.
     ★ 머리말의 「50종」 숫자는 여기서 안 고친다. usung-r8-prod-a.js / usung-r8-view.js 의
       원문과 usung-r16-i18n-{a,c,d}.js 의 한국어 키를 **같이** 50 으로 맞춰 뒀다(r45).
       r16 은 완전일치 조회라 한쪽만 고치면 5개 언어가 조용히 미번역이 된다(r20 교훈). */
  function wrap() {
