...
(function () {
   ...
   * PPT: 「대표 4종 멘트 삭제」 · 「순번대로 1,2,3,4」 · 「← 해당제품이 대표적인 부품 x」.
   * ★ 정정(r21) — 아래 기하 계산은 빨간박스가 '뱃지 줄'을 가리킨다고 결론냈지만
   *   image16.png 을 실제로 렌더해 보니 그 자리는 「대표 4종을 먼저 확인해 보세요.」
   *   문장이었고, 타일을 가리키는 건 옆의 「← 해당제품이 대표적인 부품 x」 쪽이었다.
   *   즉 지시는 '뱃지 숫자만 다시 쓰기'가 아니라 **부품 4종 자체를 1·2·3·4 로 교체**다.
   *   교체는 usung-r21.js 가 맡고(window.renderPartsTeaser → p01~p04),
   *   아래 뱃지 재기록은 '01~04' 의 앞 0 을 떼는 역할로 **그대로 유효**하다.
   *   (원 계산 기록: 직사각형 6(FF0000) x0.20" y3.19" w5.00" h0.35",
   *    image16.png 1160×487 이 x0.20" y2.63" w7.71" → 150.5px/in → x[0,752] y[84,137].)
